...
parser.add_argument('--lambda-reg', type=float, default=0.1, help='Regularization hyperparameter')
...
path = 'results/'+args.data_name+'/run'+str(args.seed)
...
if args.visualize:
    ...
    if args.transfer:
        rmse = test_once(test_graphs, model, args.batch_size, logger, device=device)
    ...
    if args.ensemble:
        ...
        print('Bottleneck Neurons:', (model.dam_layer.mask()>0).sum().item())
                
        with open(os.path.join(path, 'dam.txt'), "w") as file_object:
            str_ = '\n'+str(rmse)+'\t'+str((model.dam_layer.mask()>0).sum().item())
            file_object.write(str_)
        
    else:
        if args.transfer:
            model.load_state_dict(torch.load(args.model_pos))
            rmse = test_once(test_graphs, model, args.batch_size, device=device, logger=None)
