...
#     optimizer = Adamax(model.parameters(), lr=lr, weight_decay=weight_decay)
    ...
    for epoch in pbar:
        train_loss = train(model, optimizer, train_loader, device, lambda_reg=lambda_reg, regression=True, ARR=ARR, 
        ...
        eval_info = {
            ...
            'test_rmse': rmses[-1],
            'dam_beta': model.dam_layer.beta.item(),
        }
        if not batch_pbar:
            pbar.set_description(
                'Epoch {}, train loss {:.6f}, test rmse {:.6f}, beta {:.3f}'.format(*eval_info.values())
            )
        else:
            print('Epoch {}, train loss {:.6f}, test rmse {:.6f}, beta {:.3f}'.format(*eval_info.values()))
...
def train(model, optimizer, loader, device, lambda_reg=0.1, regression=False, ARR=0, 
    ...
    for data in pbar:
        ...
        if show_progress:
            pbar.set_description('Epoch {}, batch loss: {}, beta: {}'.format(epoch, loss.item(), model.dam_layer.beta.item()))
        if ARR != 0:
        ...
        loss = loss + lambda_reg*model.dam_layer.beta
        loss.backward()
...
def visualize(model, graphs, res_dir, data_name, device, class_values, num=5, sort_by='prediction'):
    model.eval()
    model.to(device)
    R = []
    Y = []
